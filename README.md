...
全自動體育賽事預測機器人。每小時透過 GitHub Actions 自動執行：建立今日賽事池 → AI/統計預測 → 推播 Telegram → 賽後驗證命中 → 累積訓練 XGBoost 模型。
...
|📈 EV 模型表現        |賽後自動評估 EV 準確性、Edge 命中是否合理                             |
|🏛️ 市場偏差分析         |賽後顯示模型 vs 市場差異，標記低估／高估程度                              |
|💓 推播決策報告         |無推播時發送決策統計（靜音），列出時間窗／靜默／重複／模型拒絕／發送失敗各項攔截數             |
|🚨 推播失敗 Alert     |Telegram API 失敗重試 3 次後自動發送錯誤通知                        |
|🏆 世界盃特報          |冠軍 / 金靴 / 金球 / 金手套賠率即時分析（6/11～7/19 生效）                |
...
├── worldcup_engine.py        # 世界盃每日推播模組（獨立）
...
### 3. 初始化 flags.json 與 weekly_games.json

首次部署前，在 repo 根目錄建立這兩個檔案：

```bash
echo '{}' > flags.json
echo '{"games":[],"updated_at":""}' > weekly_games.json
git add flags.json weekly_games.json
git commit -m "init state files"
git push
```

### 4. 啟用 GitHub Actions

確認 repo → **Actions** → 已啟用。Actions 每小時自動執行，**無需任何手動操作**。

> 手動測試：Actions → Sports Prediction Bot → Run workflow
...
python sports_prediction.py fetch       # 強制重抓今日賽程存入 cache
...
python sports_prediction.py --debug     # Dry-run：列出決策判斷，強制模擬前兩場
...
|時間（台灣）                       |UTC        |動作                        |
|-----------------------------|-----------|--------------------------|
|每小時整點                        |每小時整點      |Hourly Push：推播今日符合條件賽事    |
|00:00 / 06:00 / 12:00 / 18:00|16/22/04/10|今日賽事池自動刷新（data_manager 判斷）|
|每週日 21:00                    |週日 13:00   |週報 + 系統自學指標               |


> GitHub Actions cron 以 UTC 執行。刷新邏輯由 `data_manager.REFRESH_HOURS_TW` 控制，不需要額外 cron。
