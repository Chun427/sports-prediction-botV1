...
第一層：每日 06:00 TW（UTC 22:00）→ Daily Fetch
  ├─ 強制重抓今日賽程（不使用舊快取）
  ├─ 動態驗證 sport key 是否有效（避免 404 浪費重試）
  └─ 只保留今天 + 未來 3 天的賽事存入 cache

第二層：每小時整點 → Hourly Push
  ├─ 從 cache 讀取今日賽事
  ├─ 只推台灣時間「今天」的賽事（SAME_DAY_ONLY）
...
| 推播決策報告 | 靜音發送，列出時間窗／靜默／重複／模型拒絕／發送失敗各項數量 |
...
|時間            |動作                  |
|--------------|--------------------|
|每日 06:00 台灣時間 |Daily Fetch：強制重抓今日賽程|
|每小時整點         |Hourly Push：推播符合條件賽事|
|每週日 21:00 台灣時間|週報 + 系統自學指標         |


> GitHub Actions cron 以 UTC 執行，06:00 TW = UTC 22:00。
